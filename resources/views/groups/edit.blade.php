          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="form-group" id="criteria_program_group">
              ...
              <select name="criteria[program_ids][]" class="form-control select2" multiple data-placeholder="Select one or more programmes">
                ...
                <option value="{{ $program->id }}" {{ (is_array(old('criteria.program_ids', $group->criteria['program_ids'] ?? [])) && in_array($program->id, old('criteria.program_ids', $group->criteria['program_ids'] ?? []))) ? 'selected' : '' }}>[{{ $program->code }}] {{ $program->name }}</option>
                @endforeach
              </select>
