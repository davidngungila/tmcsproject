...
<div class="animate-in space-y-6">
    ...
    <!-- ADD MEMBER MODAL -->
    <div id="addMemberModal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-md z-50 flex items-center justify-center hidden" style="display: none;">
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300">
            <div class="p-10 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div>
                    <h3 class="text-xl font-black text-gray-800 tracking-tight">Add New Member</h3>
                    <p class="text-[10px] text-muted font-black uppercase tracking-[0.2em] mt-2">Select a member to join this community</p>
                </div>
                <button onclick="closeAddMemberModal()" class="w-10 h-10 rounded-2xl bg-white text-gray-400 flex items-center justify-center hover:bg-red-50 hover:text-red-500 shadow-sm border border-gray-100 transition-all">
                    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <form action="{{ route('groups.operations.members.add', $group->id) }}" method="POST" class="p-10 space-y-8">
                @csrf
                <div class="form-group">
                    <label class="form-label text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-4 block">Select Member</label>
                    <div class="relative group">
                        <select name="member_id" class="form-control w-full p-5 rounded-2xl border-gray-100 bg-gray-50/50 font-bold text-sm focus:ring-4 focus:ring-green-500/10 focus:border-green-500 transition-all select2" required>
                ...
                <button type="submit" class="btn btn-primary w-full py-5 rounded-2xl shadow-xl shadow-green-200 font-black uppercase tracking-[0.2em] text-xs hover:scale-[1.02] active:scale-[0.98] transition-all">
                    Add to Community
                </button>
            </form>
        </div>
