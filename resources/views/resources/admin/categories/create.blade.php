...
@section('title', 'Create Category - Administration')
...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold">Create Resource Category</h1>
</div>
    ...
    <div class="card-body">
        <form action="{{ route('admin.resource-categories.store') }}" method="POST">
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" name="name" class="form-control @error('name') border-red-500 @enderror" placeholder="e.g. Prayers" value="{{ old('name') }}" required>
                @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="form-group mb-4">
                <label class="form-label">Icon (Lucide/SVG name)</label>
                <input type="text" name="icon" class="form-control" placeholder="e.g. book-open" value="{{ old('icon') }}">
            </div>
