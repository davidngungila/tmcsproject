...
@section('title', 'Create Template - TmcsSmart')
@section('page-title', 'Create Template')
@section('breadcrumb', 'TmcsSmart / Communications / Templates / Create')
    ...
    <div class="lg:col-span-2">
      <div class="card">
        <div class="card-header">
          ...
          <div class="card-subtitle">Define reusable message content with placeholders</div>
        </div>
        <div class="card-body">
          <form action="{{ route('message-templates.store') }}" method="POST" id="templateForm">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div class="form-group">
                <label class="form-label">Template Name *</label>
                <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="e.g. Sunday Service Reminder">
            ...
            <div class="form-group" id="subjectGroup" style="{{ old('type') == 'Email' ? '' : 'display:none;' }}">
              <label class="form-label">Default Subject</label>
              <input type="text" name="subject" class="form-control" value="{{ old('subject') }}" placeholder="Subject line for the email">
            ...
            <div class="form-group">
              ...
              <textarea name="content" class="form-control" rows="10" required placeholder="Type the reusable content here...">{{ old('content') }}</textarea>
            ...
            <div class="flex gap-3 mt-8">
              ...
              <button type="submit" class="btn btn-primary flex-1">Save Template</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="card mb-6">
        ...
        <div class="card-body">
          <form action="{{ route('message-templates.test') }}" method="POST" id="testForm">
            @csrf
            <input type="hidden" name="type" id="testType" value="SMS">
            <input type="hidden" name="subject" id="testSubject">
            <input type="hidden" name="content" id="testContent">
