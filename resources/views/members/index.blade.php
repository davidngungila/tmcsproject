          ...
          <tr class="hover:bg-primary/5 transition-colors">
            ...
            <td class="px-6 py-4 text-right">
              <div class="flex items-center justify-end gap-1">
                ...
                @endif
                @if(!$member->is_active && auth()->user()->hasPermission('members.edit'))
                <form action="{{ route('members.approve', $member->id) }}" method="POST" class="inline">
                  @csrf
                  <button type="submit" class="p-2 rounded-lg text-muted hover:text-green-600 hover:bg-green-500/10 transition-all" title="Approve Member" onclick="return confirm('Approve this member?')">
